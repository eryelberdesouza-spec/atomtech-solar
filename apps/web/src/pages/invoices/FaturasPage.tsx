            ...
            <Card key={f.id} style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                  <span style={{ color: C.accent, fontFamily: 'monospace', fontSize: 13, fontWeight: 700 }}>{f.referencia}</span>
                  <span style={{ color: C.textDim, fontSize: 12 }}>{f.distribuidora}</span>
                </div>
                <p style={{ color: C.textDim, fontSize: 12, margin: 0 }}>UC: {f.codigoUC} · {f.tipoFornecimento}</p>
              </div>
...
// Gera o array de 12 meses no formato MM/AAAA (máximo 7 chars — limite do backend)
function gerarHistoricoInicial() {
  const agora   = new Date()
  const mesAtual = agora.getMonth() // 0-11
  const anoAtual = agora.getFullYear()
  return Array.from({ length: 12 }, (_, i) => {
    // Mês 0 = mês atual, 1 = mês anterior, ..., 11 = 11 meses atrás
...
export function NovaFaturaPage() {
  ...
  const [form, setForm] = useState({
    distribuidora:        '',
    referencia:           '',   // formato: MM/AAAA (7 chars)
    codigoUC:             '',
    codigoInstalacao:     '',
    tipoFornecimento:     '',
    classificacao:        '',
  ...
  // Tarifa calculada automaticamente
    ...
    <PageWrapper>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        <button onClick={() => navigate('/faturas')}
          style={{ background: 'none', border: 'none', color: C.textMuted, cursor: 'pointer', fontSize: 20 }}>←</button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 20, alignItems: 'start' }}>
          ...
          <Card style={{ padding: '16px 20px' }}>
            ...
            {tarifaCalculada && (
              <div style={{ marginTop: 10, padding: '8px 12px', background: `${C.green}10`, borderRadius: 7, border: `1px solid ${C.green}30` }}>
                <span style={{ color: C.green, fontSize: 12, fontWeight: 600 }}>Tarifa média calculada: R$ {tarifaCalculada}/kWh</span>
              </div>
            )}
        ...
        <Card style={{ padding: '16px 20px' }}>
          <p style={{ color: C.textMuted, fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.07em', margin: '0 0 8px' }}>
            Histórico de Consumo (12 meses)
          </p>
          <p style={{ color: C.textDim, fontSize: 11, margin: '0 0 12px' }}>
            Preencha com os dados da seção "CONSUMO kWh" da fatura. Formato: MM/AAAA.
          </p>
          {historico.map((h, i) => (
            <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
              <input
                ...
                placeholder="MM/AAAA"
                style={{
                  width: 80, padding: '6px 8px', borderRadius: 7, fontFamily: 'monospace',
                  background: C.dark, border: `1px solid ${C.darkBorder}`, color: C.textDim, fontSize: 11, outline: 'none',
                }}
              />
              <input
                type="number"
                ...
                style={{
                  ...
                  background: C.dark, border: `1px solid ${C.darkBorder}`, color: C.text, fontSize: 12, outline: 'none',
                }}
              />
              <input
                type="number"
                ...
                style={{
                  ...
                  background: C.dark, border: `1px solid ${C.darkBorder}`, color: C.textMuted, fontSize: 11, outline: 'none',
                }}
              />
            </div>
          ))}
