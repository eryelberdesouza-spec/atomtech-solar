...
function TabPagamento({ condições, propostaId }: any) {
  const utils = trpc.useUtils()
  ...
  if (!condições?.length) return <EmptyState icon="💳" title="Condições comerciais não configuradas" />

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {condições.map((c: any, i: number) => {
            ...
            {!isEditando && (c.parcelas ?? []).map((p: any, j: number) => (
              <div key={j} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderTop: `1px solid ${C.darkBorder}40` }}>
                <div>
                  ...
                  {p.prazoDias > 0 && (
                    <span style={{ color: C.textDim, fontSize: 11, marginLeft: 8 }}>(até {p.prazoDias} dias {p.tipoPrazo})</span>
                  )}
                </div>
                  ...
                  <div>
                    ...
                    {formAvista.desconto > 0 && (
                      <p style={{ color: C.green, fontSize: 12, margin: '4px 0 0', fontWeight: 600 }}>
                        → {formatCurrency(Number(c.valorTotal) * (1 - formAvista.desconto / 100))}
                      </p>
                    )}
                  </div>
                  <div>
                      ...
                      updateCond.mutate({
                        ...
                        parcelas: [{
                          ...
                          descricaoEvento: formAvista.desconto > 0
                            ? `À vista com ${formAvista.desconto}% de desconto`
                            : 'Pagamento à vista',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ padding: '12px 24px', borderBottom: `1px solid ${C.darkBorder}`, background: C.darkMid, display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <Btn size="sm" onClick={() => generatePdf.mutate({ propostaId })} disabled={generatePdf.isPending}>
            {generatePdf.isPending ? '⏳ Gerando...' : '↓ Exportar PDF'}
          </Btn>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '22px 24px' }}>
        ...
        {tab === 'pagamento'       && <TabPagamento condições={condicoesComerciais} propostaId={propostaId} />}
