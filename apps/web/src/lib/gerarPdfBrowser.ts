...
// Renderiza markdown básico: **negrito**, listas com "- ", parágrafos
...
const CSS = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Cada .page = uma página A4 completa com header e footer */
  .page {
    width: 210mm;
    min-height: 297mm;
    position: relative;
  ...
  /* Header e Footer internos (todas as páginas exceto capa) */
  .header-interno {
    background: #0E2040; padding: 13px 36px;
  ...
  .footer {
    background: #0E2040; padding: 14px 36px;
    display: flex; justify-content: space-between; align-items: center;
  ...
  /* Conteúdo */
  .section { padding: 22px 36px; flex: 1; overflow: hidden; }
  .section-title {
    font-size: 20px; font-weight: 800; color: #0E2040;
    border-left: 4px solid #F5A623; padding-left: 12px;
    margin-bottom: 14px;
  }
  .section-sub { font-size: 15px; font-weight: 700; color: #0E2040; margin: 18px 0 10px; }

  /* KPIs */
  .kpi-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin: 12px 0; }
  .kpi-card { background: #f8f9fc; border-radius: 8px; padding: 14px; border-top: 3px solid #F5A623; }
  .kpi-card-green { border-top-color: #2D9C4E; }
  .kpi-label { font-size: 10px; color: #888; letter-spacing: 2px; text-transform: uppercase; }
  .kpi-value { font-size: 22px; font-weight: 800; color: #0E2040; margin-top: 3px; }
  .kpi-unit { font-size: 11px; color: #888; font-weight: 400; }

  /* Tabela */
  table { width: 100%; border-collapse: collapse; margin: 10px 0; font-size: 12px; }
  th { background: #0E2040; color: #fff; padding: 7px 10px; text-align: left; font-size: 10px; letter-spacing: 1px; text-transform: uppercase; }
  td { padding: 7px 10px; border-bottom: 1px solid #f0f0f0; color: #333; }
  tr:nth-child(even) td { background: #fafafa; }

  /* Textos institucionais */
  p { font-size: 12px; color: #444; line-height: 1.8; margin-bottom: 10px; }
  ul { padding-left: 20px; margin: 8px 0 10px; }
  li { font-size: 12px; color: #444; line-height: 1.9; }
  strong { font-weight: 700; color: #0E2040; }

  /* Pagamento */
  .pagamento-box { border: 2px solid #F5A623; border-radius: 10px; padding: 14px; margin: 10px 0; }
  .pagamento-tipo { font-size: 13px; font-weight: 700; color: #0E2040; margin-bottom: 8px; }
  .pagamento-linha { display: flex; justify-content: space-between; padding: 5px 0; border-bottom: 1px solid #f5f5f5; font-size: 12px; }
  .pagamento-linha:last-child { border-bottom: none; }

  /* Análise financeira */
  .highlight-box { background: linear-gradient(135deg, #fff8e8, #fff3d0); border-left: 4px solid #F5A623; padding: 12px 16px; border-radius: 0 8px 8px 0; margin: 10px 0 14px; }
  ...
  .comp-label { font-size: 10px; color: #888; letter-spacing: 2px; text-transform: uppercase; }
  .comp-value { font-size: 14px; font-weight: 800; color: #0E2040; margin-top: 6px; }
  .comp-badge { font-size: 11px; color: #F5A623; font-weight: 700; margin-top: 4px; }
  ...
  /* Aceite */
  .aceite-box { background: #f8f9fc; border-radius: 10px; padding: 20px; margin: 14px 0; }
  ...
  return `
    <div class="header-interno">
      <div class="header-logo">
        ${logoUrl
          ? `<img src="${logoUrl}" style="height:26px;max-width:140px;object-fit:contain;vertical-align:middle;" alt="Logo"/>`
          : `ATOM<span>TECH</span>`
        }
...
function footer(numero: string, empresa?: any) {
  const telefone = empresa?.telefone || '(61) 3978-1738'
  const email    = empresa?.email    || 'contato@atomtech.tec.br'
  const cidade   = empresa?.cidade   || 'Bras\u00edlia/DF'
  return `
    <div class="footer">
      <div class="footer-text">${empresa?.nome ?? 'Atom Tech'} &mdash; Energia Solar e Tecnologia<br>${cidade} &middot; ${email} &middot; ${telefone}</div>
      <div class="footer-numero">${numero}</div>
    </div>
  `
}

// Renderiza um bloco de texto institucional como página completa
function paginaTexto(titulo: string, chave: string, textos: any, numero: string, logoUrl?: string, empresa?: any): string {
  ...
  return `
    <div class="page">
      ...
      ${footer(numero, empresa)}
    </div>`
}

function gerarHTML(data: any): string {
  const {
    proposta: prop, empresa: emp, cliente: cli,
    fatura: fat, dimensionamento: dim, equipamentos: equips,
    precificacao: prec, analiseFinanceira: af,
    condicoesComerciais: condicoes, blocos, textos,
  } = data
  ...
  // Se não há blocos configurados, mostra tudo (fallback padrão)
  ...
  const capa = tem('capa') ? `
    <div class="page capa">
      <div class="capa-top">
        ...
        <div style="font-size:9px;color:rgba(255,255,255,0.3);text-align:right">
          ${emp?.cidade ?? 'Bras\u00edlia'}/DF<br>www.atomtech.tec.br
        </div>
      </div>
      <div class="capa-body">
      ...
      <div class="capa-footer">
        <div class="capa-footer-left">
          ${emp?.endereco ?? 'Edif\u00edcio SIA Centro Empresarial \u2014 Sala 231 B'}<br>
          ${emp?.cidade ?? 'Bras\u00edlia'}/DF
        </div>
  ...
  const apresentacao = tem('apresentacao_empresa') ? `
    <div class="page">
      ...
      <div class="section">
        ...
        ${renderTexto(textos?.apresentacao_empresa?.conteudo) ||
          '<p>A Atom Tech \u00e9 especializada em sistemas fotovoltaicos com foco em qualidade e resultado.</p>'}
  ...
  // ── COMO FUNCIONA ────────────────────────────────────────────────────────
  const comoFunciona = tem('como_funciona')
    ? paginaTexto('Como Funciona a Energia Solar', 'como_funciona', textos, numero, logoUrl, emp)
    : ''

  // ── DIFERENCIAIS ─────────────────────────────────────────────────────────
  const diferenciais = tem('diferenciais')
    ? paginaTexto('Diferenciais da Atom Tech', 'diferenciais', textos, numero, logoUrl, emp)
    : ''

  // ── GARANTIAS ────────────────────────────────────────────────────────────
  const garantias = tem('garantias')
    ? paginaTexto('Garantias Inclusas', 'garantias', textos, numero, logoUrl, emp)
    : ''

  // ── FORNECEDORES ─────────────────────────────────────────────────────────
  const fornecedores = tem('fornecedores')
    ? paginaTexto('Fornecedores e Fabricantes', 'fornecedores', textos, numero, logoUrl, emp)
    : ''

  // ── REGULAMENTAÇÃO ───────────────────────────────────────────────────────
  const regulamentacao = tem('regulamentacao')
    ? paginaTexto('Regulamenta\u00e7\u00e3o no Brasil', 'regulamentacao', textos, numero, logoUrl, emp)
    : ''
  ...
  const dimensionamentoBloco = tem('dimensionamento') ? `
    <div class="page">
      ...
      <div class="section">
        ...
        <div class="kpi-grid">
          <div class="kpi-card">
            <div class="kpi-label">Pot\u00eancia Proposta</div>
            <div class="kpi-value">${Number(dim?.potenciaFinalKwp ?? 0).toFixed(2)} <span class="kpi-unit">kWp</span></div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">Gera\u00e7\u00e3o/M\u00eas</div>
            <div class="kpi-value">${formatKwh(Number(dim?.geracaoAnualKwh ?? 0) / 12)} <span class="kpi-unit">/m\u00eas</span></div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">\u00c1rea Necess\u00e1ria</div>
            <div class="kpi-value">${Number(dim?.areaEstimadaM2 ?? 0).toFixed(0)} <span class="kpi-unit">m\u00b2</span></div>
          </div>
          <div class="kpi-card kpi-card-green">
            <div class="kpi-label">Consumo M\u00e9dio</div>
            <div class="kpi-value">${formatKwh(dim?.consumoMedioMensalKwh)} <span class="kpi-unit">/m\u00eas</span></div>
          </div>
          <div class="kpi-card kpi-card-green">
            <div class="kpi-label">% Compensa\u00e7\u00e3o</div>
            <div class="kpi-value">${Number(dim?.percentualCompensacao ?? 0).toFixed(0)}<span class="kpi-unit">%</span></div>
          </div>
          <div class="kpi-card kpi-card-green">
            <div class="kpi-label">Economia/M\u00eas Est.</div>
            <div class="kpi-value" style="font-size:17px">${formatCurrency(dim?.economiaMensalEstimada)}</div>
          </div>
        </div>
        ...
        <div style="margin-top:18px">
          ...
          <table>
            <thead>
              <tr>
                <th>Item</th><th>Fabricante / Modelo</th>
                <th style="text-align:center">Qtd.</th><th>Pot\u00eancia</th><th>Garantia</th>
              </tr>
            </thead>
            <tbody>
              ${equips.map((eq: any) => `
    ...
    <div class="page">
      ...
      <div class="section">
        ...
        <div class="highlight-box">
          <p>Investimento de <strong>${formatCurrency(precoFinal)}</strong> com infla\u00e7\u00e3o energ\u00e9tica de 9,5% a.a. e 25 anos de vida \u00fatil.</p>
        </div>
        <div class="kpi-grid">
          <div class="kpi-card">
        ...
        <div style="margin-top:14px">
          <p style="font-size:10px;color:#888;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Saldo ap\u00f3s 25 anos \u2014 Comparativo</p>
          <div class="comparativo-grid">
            <div class="comp-card comp-poupanca">
              <div class="comp-label">Poupan\u00e7a</div>
        ...
        <div style="margin-top:18px">
          ...
          <div class="reducao-grid">
            <div>
              <div class="reducao-header">Antes da Instala\u00e7\u00e3o</div>
            ...
            <div>
              <div class="reducao-header">Economia M\u00e9dia Mensal</div>
              <div class="reducao-cell">
                <div class="reducao-label">Gera\u00e7\u00e3o Solar</div>
                <div class="reducao-value economia">${formatCurrency(af?.economiaMensalAno1)}</div>
              </div>
            </div>
            <div>
              <div class="reducao-header">Depois da Instala\u00e7\u00e3o</div>
              <div class="reducao-cell">
  ...
  const condicoesBloco = tem('condicoes_comerciais') ? `
    <div class="page">
      ...
      <div class="section">
        ...
        <div class="highlight-box">
          <p>Investimento Total: <strong>${formatCurrency(precoFinal)}</strong> &middot; Economia Estimada: <strong>${formatCurrency(af?.economiaMensalAno1)}/m\u00eas</strong></p>
        </div>
  ...
  const aceite = tem('aceite') ? `
    <div class="page">
      ...
      <div class="section">
        ...
        <p style="margin-top:18px;font-size:14px;line-height:1.9">
        ...
        <p style="margin-top:14px;font-size:13px;color:#666">Bras\u00edlia, em ${formatDate(prop?.dataEmissao)}.</p>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-top:28px">
          <div>
            <p style="font-size:12px;font-weight:700;color:#0E2040;margin-bottom:6px">Contato</p>
            <p style="font-size:11px">${emp?.telefone ?? '(61) 3978-1738'} &middot; ${emp?.email ?? 'contato@atomtech.tec.br'}</p>
          </div>
          <div>
            <p style="font-size:12px;font-weight:700;color:#0E2040;margin-bottom:6px">Endere\u00e7o</p>
            <p style="font-size:11px">${emp?.endereco ?? 'Edif\u00edcio SIA Centro Empresarial, Sala 231 B'} \u2014 ${emp?.cidade ?? 'Bras\u00edlia'}/DF</p>
          </div>
        </div>` : ''}
      </div>
      ${footer(numero, emp)}
