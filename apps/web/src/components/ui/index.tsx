// ═══════════════════════════════════════════════════════════════════
// Componentes UI reutilizáveis — design system Atom Tech
// ═══════════════════════════════════════════════════════════════════
...
export function Badge({ status }: { status: string }) {
  const s = STATUS_CONFIG[status] ?? { label: status, color: C.textMuted, bg: C.darkBorder }
  return (
    <span style={{
...
export function Card({ children, style, onClick, hover }: CardProps) {
  return (
    <div
      onClick={onClick}
      style={{
        background: C.darkCard, borderRadius: 12, border: `1px solid ${C.darkBorder}`,
        cursor: onClick ? 'pointer' : undefined,
        transition: hover ? 'border-color 0.15s' : undefined,
        ...style,
      }}
      onMouseEnter={e => hover && onClick && (e.currentTarget.style.borderColor = C.solar + '60')}
      onMouseLeave={e => hover && onClick && (e.currentTarget.style.borderColor = C.darkBorder)}
    >
      {children}
    </div>
  )
...
export function Input({ label, error, suffix, style, ...props }: InputProps) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      {label && (
        <label style={{ color: C.textDim, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 600 }}>
          {label}
        </label>
      )}
...
export function KpiCard({ label, value, sub, trend, color = C.solar }: KpiCardProps) {
  return (
    <Card style={{ padding: '18px 20px', position: 'relative', overflow: 'hidden' }}>
      ...
      <p style={{ color: C.text, fontSize: 22, fontWeight: 700, margin: '6px 0 2px', fontFamily: 'monospace' }}>
        {value}
      </p>
      {sub && <p style={{ color: C.textMuted, fontSize: 12, margin: 0 }}>{sub}</p>}
      {trend !== undefined && (
        <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ fontSize: 12, color: trend >= 0 ? C.success : C.danger, fontWeight: 600 }}>
            {trend > 0 ? '+' : ''}{trend}%
          </span>
          <span style={{ fontSize: 11, color: C.textDim }}>vs. mês anterior</span>
        </div>
      )}
    </Card>
  )
      ...
      <tbody>
        {rows.map((row, i) => (
          <tr
            ...
            style={{
              borderBottom: i < rows.length - 1 ? `1px solid ${C.darkBorder}40` : 'none',
              cursor: onRowClick ? 'pointer' : undefined,
            }}
            onMouseEnter={e => onRowClick && (e.currentTarget.style.background = `${C.darkBorder}40`)}
  ...
  return (
    <button
      ...
      style={{
        ...
        background: checked ? C.green : C.darkBorder,
...
export function Tabs({ tabs, active, onChange }: TabsProps) {
  return (
    <div style={{ display: 'flex', borderBottom: `1px solid ${C.darkBorder}`, gap: 0 }}>
      {tabs.map(tab => (
        <button
          key={tab.id}
