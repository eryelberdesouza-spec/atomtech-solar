...
- **Incidente 2026-07-31 → 2026-08-03 (mais grave que o de 09/07)**: às 22h de 31/07 a sessão entrou em loop de erro `stream:error code 503` seguido de reconexões em alta frequência (`Connection Failure` a cada 2-6s) — padrão de restrição/bloqueio do próprio WhatsApp por comportamento automatizado suspeito (não é problema de infra). Ficou ~3 dias sem conectar, e o mais grave: **mesmo um logout + tentativa de registrar sessão NOVA do zero (QR novo) falhou com o mesmo `Connection Failure`** — ou seja, quando isso acontece o bloqueio é na conexão com o WhatsApp em si, re-parear não resolve na hora. Insistir em reconectar repetidamente (`start`/`stop` em sequência rápida) provavelmente piora/estende o bloqueio — se acontecer de novo, tentar UMA vez, e se falhar, esperar horas (não minutos) antes da próxima tentativa. Se persistir por muito tempo mesmo com espera, pode ser necessário verificar o número 556139781738 diretamente no WhatsApp do celular físico, ou considerar migrar para a API oficial do WhatsApp Business em vez do WAHA/Baileys (não-oficial).
- Serviço WAHA hospedado na região "Southeast Asia" no Railway (não Brasil/US) — desconhecido se isso contribui para os bloqueios, mas é uma diferença notável a considerar se o problema persistir.
