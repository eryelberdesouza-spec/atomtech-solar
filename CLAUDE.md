...
- **Formas de recebimento único** (`FORMAS_RECEBIMENTO_UNICO` = cartão de crédito, cartão de débito, financiamento): quem parcela é o cliente junto ao terceiro e a Atom recebe o valor cheio de uma vez. Geram **UMA** linha em `parcela_pagamento` com o valor total da parte; o número de vezes vai em `parcelas_forma` e é **informativo** — o contrato escreve "em até 12x junto à operadora", não um cronograma de 12 cobranças. Antes disso o contrato listava 2ª a 13ª parcela de R$ 595,83 com prazos de 30/60/…/300 dias, como se fossem 12 obrigações a prazo perante a Atom.
- **Parcelamento no cartão pode ser COM ou SEM juros, e o sistema não guarda qual** — de propósito. O valor contratado entre Atom e cliente é o mesmo nos dois casos (a Atom recebe o valor cheio da operadora); juros, quando há, são do cliente com a administradora e não passam pela Atom. Registrar a parcela com juros faria o total do contrato deixar de bater com a proposta e com o que entra no AGF. Por isso **nunca dividir o valor da parte por N** (era o que a tela fazia: "12x de R$ 595,83") e o contrato traz a ressalva de que eventuais juros e encargos da operadora correm por conta exclusiva do CONTRATANTE e não alteram o valor contratado — texto que fica correto nas duas situações sem ninguém precisar marcar nada.
- **O modal "Gerar Contrato" não pergunta mais quando há fechamento**: as opções Cartão / Financiamento / PIX suprimem a tabela de parcelas e escrevem uma forma única — num misto PIX + cartão o contrato sairia como se fosse tudo cartão. Havendo `deFechamento`, o modal exibe um resumo em somente-leitura (descrição, cada parte com forma/parcelamento/prazo e valor, total) e o modo fica travado em `'padrao'`. Sem fechamento, as quatro opções continuam como antes. O rótulo de cada parte é montado a partir de `formaPagamento`/`parcelasForma`/`prazoDias` — `descricaoEvento` é chave interna (`fechamento_1_2`) e não serve de rótulo.
...
- O contrato de serviço saía com um preâmbulo muito mais pobre: sem representante legal, sem e-mail, sem telefone, sem inscrição estadual. Num contrato de PJ (caso real: Cienge) o preâmbulo ficava quase vazio, inclusive do lado da Atom.
- `enderecoCliente`, `enderecoEmpresa` e `fmtDoc` estavam **duplicados idênticos** nos dois arquivos; agora vivem no módulo compartilhado. `fmtCnpj` do gerador de serviço virou alias de `fmtDoc`.
- **Cliente PJ tem representante legal** (`cliente.responsavel_cargo/_cpf/_email/_telefone`, migração `GET /run-migration-cliente-responsavel`; `nome_responsavel` já existia). São os dados de quem assina pela empresa — `email`/`telefone` do cliente seguem sendo os DA EMPRESA. Formulário só aparece para `tipoPessoa === 'juridica'`.
- Bloco de assinatura: CONTRATANTE PJ leva razão social + CNPJ acima da linha e nome/CPF do representante sob ela (antes a empresa "assinava" com o CNPJ no campo de CPF). No contrato de serviço, a CONTRATADA ganhou CNPJ e as linhas dos dois sócios, que não existiam.
- Campo vazio nunca vira rótulo órfão (helper `trecho()`), então cadastro incompleto só encurta a frase. **Sintoma de cadastro, não de código**: preâmbulo sem CNPJ do cliente = `cpf_cnpj` vazio no cadastro.
...
**RESOLVIDO em 2026-08-23 — PDF solar migrado pro header/footer nativo do Puppeteer**: a pendência acima (rodapé preso no meio da página quando uma seção transbordava, e cada bloco forçando página própria mesmo cabendo junto do anterior — sintoma relatado: "Condições Comerciais" descolada em AT-2026-08196) foi corrigida migrando `gerarPdfBrowser.ts` pro mesmo mecanismo já em produção no de serviço desde a rodada 7: `page.pdf({headerTemplate, footerTemplate})`, sem a tabela de repetição de `thead`/`tfoot` nem preenchimento medido em JS. `PropostaDetailPage.tsx` agora chama `gerarHTML(dados, {serverSide:true})` (retorna `{bodyHtml, capaHtml, headerTemplate, footerTemplate}`, mesmo formato do serviço) em vez da string única antiga. Caminho antigo (`window.print()`, fallback se a API cair) preservado sem mudança de comportamento — `gerarHTML` sem `serverSide` continua retornando a string HTML completa de sempre.

**Dados do cliente no PDF (2026-09-10)**: capa ganhou linha com bairro + cidade/UF sob o nome, e o Resumo ganhou bloco com CPF/CNPJ, responsável, telefone, e-mail, endereço, bairro, cidade/UF e CEP — nos dois geradores. O cadastro completo **já chegava** ao gerador (`cliente.byId` faz `select()` sem projeção e o PropostaDetailPage passa em `dadosPdf.cliente`); só não era exibido. Campo vazio não gera rótulo, e sem bairro/cidade a linha da capa não sai (a margem de 12mm do nome vai pra linha nova via style inline).

**Armadilha de bloco atômico ao inserir conteúdo (2026-09-10)**: acrescentar altura numa página empurra blocos `break-inside: avoid` INTEIROS pra página seguinte. O bloco de dados do cliente fez o info-box do escopo (itens a-e) não caber mais, e a página do Resumo ficou 60% vazia. Correção: `break-inside: auto` **só** naquele bloco, que é longo — os itens dentro dele já são protegidos individualmente. Regra geral: bloco longo deve fluir; atômico só o que é curto e não pode partir (assinatura, linha de tabela, card).
...
- **Reincidência em 2026-09-11, agora em COMPARAÇÃO de datas** (`titulo.list` e `dashboard`): `p.vencimento < hoje` com `hoje` string produz `NaN`, e `NaN` em comparação é **sempre false** — não "às vezes errado", nunca verdadeiro. Efeito: filtro "Vencidas" retornava 0 com 12 parcelas atrasadas, "Em Aberto" trazia as vencidas junto, o badge nunca exibia "Vencida" e `vencidos`/`vencendoHoje`/`vencendo7Dias` do painel viviam zerados. Sempre `fmtDateISO()` nos dois lados antes de comparar. **Sintoma típico**: um contador que nunca sai do zero, ou um filtro de status que nunca acha nada.
- **Lição de cascata (mesmo dia)**: consertar isso quebrou o alerta de atrasados da tela de Lançamentos, que consultava `status: 'ABERTA'` e só funcionava porque o bug fazia "ABERTA" devolver as vencidas junto. Ao mexer em filtro/status compartilhado, procurar quem mais consome aquele filtro — havia código dependendo do comportamento errado.
